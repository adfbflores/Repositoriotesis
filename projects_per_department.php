...
if (isset($_GET['id'])) {
    ...
    if ($qry->num_rows > 0) {
        ...
        echo "<script> alert('ID de departamento desconocido'); location.replace('./') </script>";
        exit; // Salir del script después de redireccionar
    }
} else {
    echo "<script> alert('Se requiere ID del departamento'); location.replace('./') </script>";
    exit; // Salir del script después de redireccionar
}
?>
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body rounded-0">
                <h2>Archivos de <?= isset($department['name']) ? $department['name'] : "" ?> </h2>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <ul class="pagination pagination-sm m-0 float-right">
                                ...
                                <?php endfor; ?>
                                <li class="page-item"><a class="pagelink" href="./?page=projects_per_department&id=<?= $id ?>&p=<?= $page + 1 ?>" <?= $page == $pages || $pages <= 1 ? 'disabled' : '' ?>>»</a></li>
                                        </ul>
                        </div>
                            </div>
                                </div>
